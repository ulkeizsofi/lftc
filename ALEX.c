...
typedef int (*stateHandler)(Token** tk, char* ch, int length, enum code cod, int line);
typedef int stateID;
...
int main(){
	...
	FILE* file = openFile(INPUT_STREAM);
	...
	while((nRead = readFromFile(file, input)) != 0){
		...
		while( count != nRead){
			if (currentState->handler){
				...
				currentState->handler(&tk, &input[tokenStart], count - tokenStart, 
